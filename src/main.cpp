...
double getParity(PropertyExtractor::Diagonalizer& pe, unsigned sizeX, unsigned sizeY){
	...
	for(unsigned x = 0; x < sizeX; ++x){
		for(unsigned y = 0; y < sizeX; ++y){
			...
			n_up += real(pe.calculateExpectationValue({x, y, 0, 0}, {x, y, 0, 0}));
			n_down += real(pe.calculateExpectationValue({x, y, 1, 0}, {x, y, 1, 0}));
			h_up += real(pe.calculateExpectationValue({x, y, 0, 1}, {x, y, 0, 1}));
			h_down += real(pe.calculateExpectationValue({x, y, 1, 1}, {x, y, 1, 1}));
		}
	}
